...
public class DataUpdater {
    ...
    private Map<Integer, Double> fallback = new HashMap<>();
    private Map<Integer, Integer> accessFallback = new HashMap<>();
    ...
    public void feed(RoadData data, boolean auto) {
        writeLock.lock();
        try {
            lockedFeed(data, auto);
        } finally {
            writeLock.unlock();
        }
    }

    private void lockedFeed(RoadData data, boolean auto) {
        currentRoads = data;
        ...
        for(int edgeId : accessFallback.keySet()){
            EdgeIteratorState edge = graph.getEdgeIteratorState(edgeId, Integer.MIN_VALUE);
            int direction = accessFallback.get(edgeId);
            boolean forward, backward;
            forward = direction==0||direction==2?true:false;
            backward = direction==1||direction==2?true:false;
            edge.setFlags(carEncoder.setAccess(edge.getFlags(), forward, backward));
        }
        for(int edgeId : fallback.keySet()){
            EdgeIteratorState edge = graph.getEdgeIteratorState(edgeId, Integer.MIN_VALUE);
            double oldSpeed = carEncoder.getSpeed(edge.getFlags());
            double value = fallback.get(edgeId);
            edge.setFlags(carEncoder.setSpeed(edge.getFlags(), value));
        }
        fallback = new HashMap<Integer, Double>();
        accessFallback = new HashMap<Integer, Integer>();
            ...
            if ("replace".equalsIgnoreCase(entry.getMode())) {
                if ("speed".equalsIgnoreCase(entry.getValueType())) {
                    ...
                    if (oldSpeed != value) {
                        ...
                        if(!auto){
                            if(value<5){
                                boolean forward = carEncoder.isForward(edge.getFlags());
                                boolean backward = carEncoder.isBackward(edge.getFlags());
                                int direction = -1;
                                if(forward){
                                    direction = 0;
                                }
                                ...
                                accessFallback.put(edgeId, direction);
                            }
                            fallback.put(edgeId, oldSpeed);
                        }
        ...
        logger.info("Mode: " + auto + "Updated " + updates + " street elements of " + data.size() + ". Unchanged:" + (data.size() - updates) + ", errors:" + errors);
    }
